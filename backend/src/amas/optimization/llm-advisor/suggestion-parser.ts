...
export class SuggestionParser {
  ...
  private validateSuggestionItem(item: SuggestionItem, index: number): string[] {
    ...
    const validTypes: SuggestionType[] = ['param_bound', 'threshold', 'reward_weight', 'safety_threshold'];
    ...
    if (item.currentValue !== 0) {
      const changeRatio = Math.abs(item.suggestedValue - item.currentValue) / Math.abs(item.currentValue);
      if (changeRatio > 0.5) {
  ...
  private normalizeStructure(parsed: unknown): LLMSuggestion {
    ...
    return {
      ...
      suggestions: Array.isArray(obj.suggestions)
        ? obj.suggestions.map((s: unknown, i: number) => this.normalizeSuggestionItem(s as Record<string, unknown>, i))
        : [],
