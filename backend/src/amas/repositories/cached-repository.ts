...
import { choleskyDecompose, createRegularizedIdentity } from '../common/matrix-utils';
...
export class CachedModelRepository implements ModelRepository {
  ...
  async loadModel(userId: string): Promise<BanditModel | null> {
    try {
      // 1. 先查缓存
      if (this.cacheEnabled) {
        const cached = await redisCacheService.getUserModel<VersionedCacheData<SerializedBanditModel>>(userId);
        if (cached?.data) {
