...
router.get('/range', async (req: AuthRequest, res, next) => {
    try {
        ...
        const scores = await wordScoreService.getWordsByScoreRange(userId, minScore, maxScore);
...
router.get('/stats/overview', async (req: AuthRequest, res, next) => {
    try {
        ...
        const stats = await wordScoreService.getUserScoreStats(userId);
...
router.put('/:wordId', validateWordScoreUpdate, async (req: AuthRequest, res, next) => {
    try {
        ...
        const score = await wordScoreService.upsertWordScore(userId, wordId, updateData);
