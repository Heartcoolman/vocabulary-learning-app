//! ACT-R Memory Model - Pure Rust Implementation
//!
//! Core theory:
...
/// ACT-R Memory Model Implementation
///
/// Use cases:
...
impl ACTRMemory {
    ...
    pub fn compute_activation(&self, traces: &[MemoryTrace], current_time: f64) -> f64 {
        compute_activation_internal(traces, current_time, self.state.decay)
    }
    ...
    pub fn compute_activation_with_decay(
        ...
        compute_activation_internal(traces, current_time, decay)
    }
    ...
    /// # Returns
    /// Activation value (can be -Infinity for empty traces)
    pub fn compute_activation_from_seconds_ago(&self, traces: &[MemoryTrace]) -> f64 {
        compute_activation_from_seconds_ago_internal(traces, self.state.decay)
    }
    ...
    pub fn retrieval_probability(&self, activation: f64) -> f64 {
        compute_recall_probability_internal(
            activation,
    ...
    pub fn retrieval_probability_with_params(
        ...
        compute_recall_probability_internal(activation, threshold, noise_scale)
    }
    ...
    pub fn compute_personalized_decay(
        ...
        compute_personalized_decay_internal(
            self.state.decay,
            memory_factor,
            speed_factor,
            stability_factor,
    ...
    pub fn compute_full_activation_with_decay(
        ...
        let base_activation = compute_activation_from_seconds_ago_internal(traces, decay);
        ...
        let recall_probability = compute_recall_probability_internal(
            activation,
            self.state.threshold,
            self.state.noise_scale,
        );
    ...
    pub fn predict_recall(&self, traces: &[MemoryTrace]) -> RecallPrediction {
        ...
        let activation = compute_activation_from_seconds_ago_internal(traces, self.state.decay);
        let recall_probability = compute_recall_probability_internal(
            activation,
            self.state.threshold,
            self.state.noise_scale,
    ...
    pub fn batch_compute_activations(&self, inputs: &[BatchComputeInput]) -> Vec<BatchComputeResult> {
        ...
        inputs
            ...
            .map(|input| {
                let activation =
                    compute_activation_internal(&input.traces, input.current_time, decay);
    ...
    pub fn batch_compute_activations_from_seconds_ago(
        ...
        trace_sets
            .par_iter()
            .map(|traces| {
                let activation = compute_activation_from_seconds_ago_internal(traces, decay);
                let recall_probability =
                    compute_recall_probability_internal(activation, threshold, noise_scale);
    ...
    ///
    /// This method enables action selection based on ACT-R memory model principles.
    ...
    /// # Arguments
    /// * `state` - User state
    /// * `actions` - Action parameters (JSON array string)
    ...
    pub fn select_action_from_serialized(
        ...
        let scores: Vec<f64> = parsed_actions
            .iter()
            .map(|action| compute_action_score(state, action, context))
            .collect();
    ...
    /// Select action from parsed actions
    pub fn select_action(
        &self,
        state: &ACTRSelectionState,
        actions: &[ACTRActionParams],
        context: &ACTRSelectionContext,
    ) -> ACTRSelectionResult {
        if actions.is_empty() {
            return ACTRSelectionResult {
...
pub fn compute_activation_internal(
    traces: &[MemoryTrace],
    current_time: f64,
    decay: f64,
) -> f64 {
...
pub fn compute_activation_from_seconds_ago_internal(traces: &[MemoryTrace], decay: f64) -> f64 {
    if traces.is_empty() {
        return f64::NEG_INFINITY;
    }
...
pub fn compute_recall_probability_internal(
    activation: f64,
    threshold: f64,
    noise_scale: f64,
) -> f64 {
...
/// Compute personalized decay rate based on cognitive profile
pub fn compute_personalized_decay_internal(
    base_decay: f64,
    memory_factor: f64,
    speed_factor: f64,
    stability_factor: f64,
...
pub fn compute_optimal_interval_static(
    traces: &[MemoryTrace],
    target: f64,
    decay: f64,
    threshold: f64,
    ...
    let current_activation = compute_activation_from_seconds_ago_internal(traces, decay);
    if !current_activation.is_finite() {
        return 0.0;
    }

    let current_prob = compute_recall_probability_internal(current_activation, threshold, noise_scale);
    ...
    for _ in 0..MAX_SEARCH_ITERATIONS {
        ...
        let future_activation = compute_activation_from_seconds_ago_internal(&future_traces, decay);
        let future_prob = compute_recall_probability_internal(future_activation, threshold, noise_scale);
...
pub fn compute_activation(traces: &[MemoryTrace], decay: Option<f64>) -> f64 {
    ...
    compute_activation_from_seconds_ago_internal(traces, d)
}
...
pub fn compute_recall_probability(
    ...
    compute_recall_probability_internal(activation, t, s)
}
...
mod tests {
    ...
    fn create_default_model() -> ACTRMemory {
        ACTRMemory::new(None, None, None)
    }
    ...
    fn test_custom_initialization() {
        let model = ACTRMemory::new(Some(0.6), Some(0.4), Some(0.3));
        let state = model.get_state();
    ...
    fn test_empty_traces_returns_neg_infinity() {
        let model = create_default_model();
        let activation = model.compute_activation_from_seconds_ago(&[]);
    ...
    fn test_optimal_interval_empty_traces() {
        let model = create_default_model();
        let interval = model.compute_optimal_interval(&[], 0.9);
    ...
    fn test_batch_compute_activations() {
        ...
        let results = model.batch_compute_activations_from_seconds_ago(&trace_sets);
        ...
        for (i, traces) in trace_sets.iter().enumerate() {
            let single_activation = model.compute_activation_from_seconds_ago(traces);
    ...
    fn test_batch_compute_optimal_intervals() {
        ...
        let intervals = model.batch_compute_optimal_intervals(&trace_sets, 0.9);

        assert_eq!(intervals.len(), 2);

        // Verify each result matches individual computation
        for (i, traces) in trace_sets.iter().enumerate() {
            let single_interval = model.compute_optimal_interval(traces, 0.9);
    ...
    fn test_full_activation_empty_traces() {
        let model = create_default_model();
        let result = model.compute_full_activation(&[]);
    ...
    fn test_standalone_compute_activation() {
        ...
        let activation1 = compute_activation(&traces, None);
    ...
    fn test_memory_strength_empty_traces() {
        let model = create_default_model();
        let strength = model.compute_memory_strength(&[]);
    ...
    fn test_activation_formula_precision() {
        // Test: A = ln(sum w_j * t_j^(-d))
        // For single correct review at t=100, d=0.5:
        // sum = 1.0 * 100^(-0.5) = 0.1
        // A = ln(0.1) = -2.302585

        let model = ACTRMemory::new(Some(0.5), Some(0.3), Some(0.4));
        let traces = vec![MemoryTrace {
            timestamp: 100.0,
            is_correct: true,
        }];
    ...
    fn test_recall_probability_formula_precision() {
        // Test: P = 1 / (1 + exp(-(A-tau)/s))
        // For A=0.5, tau=0.3, s=0.4:
        // z = (0.5 - 0.3) / 0.4 = 0.5
    ...
    fn test_action_selection_basic() {
        ...
        let context = ACTRSelectionContext {
            current_time: 0.0,
        ...
        let result = model.select_action(&state, &actions, &context);
    ...
    #[test]
    fn test_action_selection_high_fatigue_prefers_easy() {
        let model = create_default_model();
        let state = ACTRSelectionState {
            attention: 0.5,
        ...
        // Should prefer easy (index 0) when fatigued
