...
//!
//! Features:
//! - 22-dimensional feature vectors
//! - Cholesky decomposition for efficient updates
//! - Rank-1 updates with numerical stability
...
impl LinUCB {
    ...
    /// 构建 22 维特征向量
    pub fn build_feature_vector_internal(
        &self,
        state: &UserState,
        action: &Action,
    ...
    /// 计算 UCB 统计信息
    pub fn compute_ucb_stats_internal(&self, x: &[f64]) -> UCBStats {
        // θ = A^{-1} b = L^{-T} L^{-1} b
    ...
    pub fn build_feature_vector_typed_internal(
        &self,
        state: &UserState,
        action: &ActionTyped,
        context: &LinUCBContext,
    ...
    pub fn select_action_batch(
        ...
        contexts: &[LinUCBContext],
    ) -> Vec<ActionSelection> {
        let len = states.len().min(actions_list.len()).min(contexts.len());
        let mut results = Vec::with_capacity(len);

        for i in 0..len {
            let result = self.select_action(&states[i], &actions_list[i], &contexts[i]);
            results.push(result);
        }
    ...
    /// 使用特征向量更新
    pub fn update_with_feature_vector(&mut self, feature_vec: &[f64], reward: f64) {
        let mut x = feature_vec.to_vec();
        self.update_with_feature_vector_internal(&mut x, reward);
    }

    /// 批量更新
    pub fn update_batch(&mut self, feature_vecs: &[Vec<f64>], rewards: &[f64]) -> u32 {
        let mut success_count = 0u32;
        for (x, &r) in feature_vecs.iter().zip(rewards.iter()) {
            let mut x = x.clone();
            if x.len() == self.d {
    ...
    /// 内部更新实现
    pub fn update_with_feature_vector_internal(&mut self, x: &mut [f64], reward: f64) {
        if x.len() != self.d {
            return;
        }
    ...
    pub fn reset(&mut self) {
        *self = LinUCB::new(Some(self.alpha), Some(self.lambda));
    }
...
mod tests {
    ...
    fn test_select_action_empty_list_no_panic() {
        let linucb = LinUCB::new(None, None);
    ...
    /// 测试: 冷启动 alpha 计算
    #[test]
    fn test_cold_start_alpha() {
        // 新用户，少量交互
        let alpha1 = LinUCB::get_cold_start_alpha(5, 0.5, 0.0);
        assert!(alpha1 > 0.5); // 应该有较高探索率

        // 经验用户
        let alpha2 = LinUCB::get_cold_start_alpha(300, 0.7, 0.0);
        assert!(alpha2 < alpha1); // 探索率应该降低

        // 疲劳用户
        let alpha3 = LinUCB::get_cold_start_alpha(100, 0.7, 0.5);
        let alpha4 = LinUCB::get_cold_start_alpha(100, 0.7, 0.0);
        assert!(alpha3 < alpha4); // 疲劳时探索率降低
    }

    /// 测试: 模型更新
    #[test]
    fn test_model_update() {
        let mut linucb = LinUCB::new(None, None);
        let state = create_test_state();
        let context = create_test_context();
        let action = create_test_action("word1", "recognition");

        assert_eq!(linucb.update_count(), 0);

        linucb.update(&state, &action, 1.0, &context);

        assert_eq!(linucb.update_count(), 1);
        assert!(linucb.self_test());
    }

    /// 测试: 模型序列化
    #[test]
    fn test_model_serialization() {
        let linucb = LinUCB::new(Some(0.5), Some(2.0));
        let model = linucb.get_model();

        assert_eq!(model.alpha, 0.5);
        assert_eq!(model.lambda, 2.0);
        assert_eq!(model.d, FEATURE_DIMENSION as u32);
    }

    /// 测试: 模型恢复
    #[test]
    fn test_model_restore() {
        let mut linucb1 = LinUCB::new(None, None);
        let state = create_test_state();
        let context = create_test_context();
        let action = create_test_action("word1", "recognition");

        // 更新第一个模型
        linucb1.update(&state, &action, 1.0, &context);
        let model = linucb1.get_model();

        // 创建新模型并恢复状态
        let mut linucb2 = LinUCB::new(None, None);
        linucb2.set_model(model);

        assert_eq!(linucb1.update_count(), linucb2.update_count());
        assert_eq!(linucb1.alpha(), linucb2.alpha());
    }
}
