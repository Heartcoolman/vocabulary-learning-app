...
export const MotivationCard: React.FC<MotivationCardProps> = ({ streak, level, trend }) => {
  const getTrendIcon = () => {
    ...
    return <Minus size={20} className="text-gray-400 dark:text-gray-500" />;
  };
  ...
  return (
    <div className="group relative overflow-hidden rounded-card border border-gray-100 bg-white p-6 shadow-soft transition-all duration-g3-normal hover:shadow-elevated dark:border-slate-700 dark:bg-slate-800">
      ...
      <div className="relative z-10">
        ...
        <h3 className="mb-1 text-lg font-bold text-gray-800 dark:text-white">动机追踪</h3>
        <div className="mb-4 flex h-10 items-center gap-2">
          <span className="text-2xl font-bold text-gray-700 dark:text-gray-300">
            {getLevelLabel(level)}
          </span>
          <div className="flex items-center rounded bg-gray-50 px-2 py-1 text-xs font-medium dark:bg-slate-700">
            {getTrendIcon()}
            <span className="ml-1">
        ...
        <div className="border-t border-gray-50 pt-4 dark:border-slate-700">
          ...
          <div className="mt-1 flex justify-between">
            <span className="text-[10px] text-gray-400 dark:text-gray-500">低</span>
            <span className="text-[10px] text-gray-400 dark:text-gray-500">高</span>
          </div>
        </div>
      </div>
    </div>
