...
describe('useTrendAnalysis', () => {
  ...
  describe('useCurrentTrend', () => {
    it('should fetch current trend successfully', async () => {
      ...
      vi.spyOn(ApiClient, 'getCurrentTrend').mockResolvedValue(mockTrend as any);
    ...
    it('should auto-refresh every minute', async () => {
      vi.spyOn(ApiClient, 'getCurrentTrend').mockResolvedValue({} as any);
  ...
  describe('useTrendHistory', () => {
    it('should fetch trend history successfully', async () => {
      const mockHistory = {
        daily: [
          ...
          { date: '2024-01-02', avgAccuracy: 0.90, avgMotivation: 0.85 },
        ],
        weekly: [],
        totalDays: 28,
      };

      vi.spyOn(ApiClient, 'getTrendHistory').mockResolvedValue(mockHistory as any);
    ...
    it('should support custom days parameter', async () => {
      vi.spyOn(ApiClient, 'getTrendHistory').mockResolvedValue({} as any);
  ...
  describe('useTrendReport', () => {
    it('should fetch trend report successfully', async () => {
      ...
      vi.spyOn(ApiClient, 'getTrendReport').mockResolvedValue(mockReport as any);
  ...
  describe('useIntervention', () => {
    it('should fetch intervention suggestions successfully', async () => {
      ...
      vi.spyOn(ApiClient, 'getIntervention').mockResolvedValue(mockIntervention as any);
    ...
    it('should auto-refresh every 5 minutes', async () => {
      vi.spyOn(ApiClient, 'getIntervention').mockResolvedValue({} as any);
  ...
  describe('useGoldenTime', () => {
    it('should fetch golden time successfully', async () => {
      ...
      vi.spyOn(ApiClient, 'getGoldenTime').mockResolvedValue(mockGoldenTime as any);
  ...
  describe('useTimePreferences', () => {
    it('should fetch time preferences successfully', async () => {
      const mockPreferences = {
        ...
        averageAccuracyByHour: { 9: 0.85, 10: 0.90 },
      };

      vi.spyOn(ApiClient, 'getTimePreferences').mockResolvedValue(mockPreferences as any);
  ...
  describe('useStateHistory', () => {
    it('should fetch state history successfully', async () => {
      ...
      vi.spyOn(ApiClient, 'getStateHistory').mockResolvedValue(mockHistory as any);
  ...
  describe('useCognitiveGrowth', () => {
    it('should fetch cognitive growth successfully', async () => {
      const mockGrowth = {
        current: { memory: 0.85, speed: 0.90, stability: 0.80 },
        past: { memory: 0.75, speed: 0.80, stability: 0.70 },
        changes: {
          memory: { value: 0.10, percent: 13.3, direction: 'up' as const },
          speed: { value: 0.10, percent: 12.5, direction: 'up' as const },
          stability: { value: 0.10, percent: 14.3, direction: 'up' as const },
        },
        period: 30,
        periodLabel: '最近30天',
      };

      vi.spyOn(ApiClient, 'getCognitiveGrowth').mockResolvedValue(mockGrowth as any);
  ...
  describe('useSignificantChanges', () => {
    it('should fetch significant changes successfully', async () => {
      ...
      vi.spyOn(ApiClient, 'getSignificantChanges').mockResolvedValue(mockChanges as any);
