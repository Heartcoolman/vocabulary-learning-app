...
export class PriorityQueueScheduler {
  ...
  generateLearningQueue(
    ...
    const newWordCount = Math.min(Math.round(targetCount * newWordRatio), newWords.length);
    const reviewWordCount = Math.min(targetCount - newWordCount, reviewWords.length);

    // 如果复习单词不足，用新单词补充
    const actualNewWordCount =
      newWordCount + Math.max(0, targetCount - newWordCount - reviewWordCount);
  ...
  getDueWords(wordStates: WordLearningState[]): string[] {
    ...
    return wordStates
      .filter(
        (state) =>
          state.nextReviewDate && state.nextReviewDate <= now && state.state !== WordState.NEW,
      )
