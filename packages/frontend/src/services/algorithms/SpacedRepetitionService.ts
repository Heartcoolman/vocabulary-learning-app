...
export class SpacedRepetitionService {
  ...
  constructor(config: AlgorithmConfig, storage: WordStateStorage) {
  ...
  async submitAnswer(
    ...
    // 计算正确率
    const accuracy =
      wordScore.totalAttempts > 0 ? wordScore.correctAttempts / wordScore.totalAttempts : 0;
  ...
  async endSession(): Promise<SessionInfo | null> {
    ...
    const accuracy = totalWords > 0 ? this.currentSession.correctCount / totalWords : 0;
   ...
   * @returns 重置后的单词状态
   */
  async resetProgress(userId: string, wordId: string, reason?: string): Promise<WordLearningState> {
    // 重新初始化单词状态
    const newState = await this.stateManager.initializeWordState(userId, wordId);
