...
pub async fn generate_report(proxy: &DatabaseProxy) -> Result<WeeklyReport, String> {
    let end = Utc::now();
...
async fn generate_llm_analysis(
    ...
    crate::services::llm_provider::LLMError,
> {
...
> {
    ...
    let normalized_recommendations: Vec<Recommendation> = parsed
        .recommendations
        .into_iter()
    ...
    let normalized_concerns: Vec<Concern> = parsed
        .concerns
        .into_iter()
        .map(|c| Concern {
            title: c.title,
...
fn generate_heuristic_analysis(
    ...
    let summary = format!(
        ...
        metrics.learning.total_answers,
        metrics.learning.avg_accuracy * 100.0
    );
...
async fn store_report(proxy: &DatabaseProxy, report: &WeeklyReport) -> Result<(), String> {
    ...
    let learning_metrics_value =
        serde_json::to_value(&report.key_metrics.learning).unwrap_or_default();
...
pub async fn get_health_trend(
    proxy: &DatabaseProxy,
    weeks: i32,
) -> Result<Vec<HealthTrendPoint>, String> {
    let pool = proxy.pool();
    ...
    let result: Vec<HealthTrendPoint> = rows
        .into_iter()
        .map(|r| {
            let ws: chrono::NaiveDateTime = r
                .try_get("weekStart")
                .unwrap_or_else(|_| Utc::now().naive_utc());
            let health_score = r
                .try_get::<f64, _>("healthScore")
...
fn parse_report_pg(row: &sqlx::postgres::PgRow) -> WeeklyReport {
    let key_metrics: KeyMetrics = row
        .try_get::<serde_json::Value, _>("keyMetrics")
        .ok()
        .and_then(|v| serde_json::from_value(v).ok())
        .unwrap_or_else(default_metrics);
    let highlights: Vec<Highlight> = row
        .try_get::<serde_json::Value, _>("highlights")
        .ok()
        .and_then(|v| serde_json::from_value(v).ok())
        .unwrap_or_default();
    let concerns: Vec<Concern> = row
        .try_get::<serde_json::Value, _>("concerns")
        .ok()
        .and_then(|v| serde_json::from_value(v).ok())
        .unwrap_or_default();
    let recommendations: Vec<Recommendation> = row
        .try_get::<serde_json::Value, _>("recommendations")
        .ok()
        .and_then(|v| serde_json::from_value(v).ok())
        .unwrap_or_default();
    ...
    let health_score = row
        .try_get::<i32, _>("healthScore")
        .or_else(|_| {
            row.try_get::<f64, _>("healthScore")
                .map(|v| v.round() as i32)
        })
        .unwrap_or(0)
        .clamp(0, 100);
