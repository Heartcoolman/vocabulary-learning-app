...
pub async fn batch_get(
    ...
    let map = match word_states::batch_get_word_states(proxy.as_ref(), request_state, &auth_user.id, &unique_ids).await {
        Ok(value) => value,
        Err(err) => {
...
pub async fn due_list(
    ...
    match word_states::list_due_words(proxy.as_ref(), request_state, &auth_user.id).await {
...
pub async fn stats_overview(
    ...
    match word_states::get_state_stats(proxy.as_ref(), request_state, &auth_user.id).await {
...
pub async fn by_state(
    ...
    match word_states::list_words_by_state(proxy.as_ref(), request_state, &auth_user.id, state_value).await {
        Ok(words) => Json(SuccessResponse { success: true, data: words }).into_response(),
        Err(err) => {
...
pub async fn get_one(
    ...
    match word_states::get_word_state(proxy.as_ref(), request_state, &auth_user.id, &word_id).await {
...
pub async fn upsert_one(
    ...
    match word_states::upsert_word_state(proxy.as_ref(), request_state, &auth_user.id, &word_id, update).await {
...
pub async fn delete_one(
    ...
    match word_states::delete_word_state(proxy.as_ref(), request_state, &auth_user.id, &word_id).await {
