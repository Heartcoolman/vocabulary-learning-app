...
/// Shadow calculation result for UMM vs FSRS comparison
...
pub struct ShadowResult {
    ...
    pub umm_retrievability: Option<f64>,
    pub umm_interval: Option<f64>,
}

pub struct UmmEngine;
...
impl UmmEngine {
    ...
    /// Compute shadow results for A/B comparison
    /// Runs UMM algorithms in parallel with FSRS without affecting actual scheduling
...
mod tests {
    ...
    fn test_basic_retrievability() {
        ...
        let r = UmmEngine::compute_retrievability(&state, 0.0, &[], &[], &[], &[]);
    ...
    fn test_mtp_bonus_increases_r() {
        ...
        let r_base = UmmEngine::compute_retrievability(&state, 1.0, &[], &[], &[], &[]);
        let r_with_mtp = UmmEngine::compute_retrievability(&state, 1.0, &morphemes, &[], &[], &[]);
    ...
    fn test_iad_penalty_decreases_r() {
        ...
        let r_base = UmmEngine::compute_retrievability(&state, 1.0, &[], &[], &[], &[]);
        let r_with_iad = UmmEngine::compute_retrievability(&state, 1.0, &[], &pairs, &recent, &[]);
    ...
    fn test_interval_computation() {
        let state = MdmState::default();
        let interval = UmmEngine::compute_interval(&state, 0.9, &[], &[], &[], &[]);
        assert!(interval > 0.0);
    }
    ...
    #[test]
    fn test_umm_computation_performance() {
