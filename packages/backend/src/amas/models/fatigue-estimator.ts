...
import { FatigueRecoveryModel } from './cognitive';
...
export class FatigueEstimator {
  ...
  constructor(params = DEFAULT_FATIGUE_PARAMS, initialFatigue: number = 0.1) {
  ...
  update(features: FatigueFeatures): number {
    ...
    const rawBreakMinutes = features.breakMinutes ?? (now - this.lastUpdateTime) / 60000;
  ...
  updateFromEvent(
    ...
    const rt_increase_rate = Math.min(1.0, Math.max(0, (responseTime - baselineRT) / baselineRT));
