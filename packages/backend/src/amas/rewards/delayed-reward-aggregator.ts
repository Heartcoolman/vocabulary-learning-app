...
const DEFAULT_SCHEDULE: RewardSchedule[] = [
  ...
  { delaySec: 3600, weight: 0.2, label: '1h' }, // 1小时
  { delaySec: 21600, weight: 0.15, label: '6h' }, // 6小时
  { delaySec: 86400, weight: 0.2, label: '24h' }, // 24小时
...
export class DelayedRewardAggregator {
  ...
  addReward(
    ...
    // 生成唯一ID
    const eventId = options?.id ?? `reward-${userId}-${timestamp}-${this.idSequence++}`;
  ...
  private pruneQueue(): void {
    ...
    const eventsWithProgress = this.queue.map((event) => {
      ...
      const totalDelivered = event.delivered.reduce((sum, d) => sum + Math.abs(d), 0);
    ...
    const eventsToKeep = eventsWithProgress.slice(-targetSize).map((e) => e.event);
  ...
  aggregate(now = Date.now(), userId?: string): AggregatedResult {
    ...
    for (const event of this.queue) {
      ...
      for (let i = 0; i < this.schedule.length; i++) {
        ...
        // 计算进度 (即时奖励进度为1)
        const progress = delaySec <= 0 ? 1 : this.clamp(elapsedSec / delaySec, 0, 1);
  ...
  setState(state: DelayedRewardState): void {
    ...
    if (state.version !== DelayedRewardAggregator.VERSION) {
      amasLogger.debug(
        { from: state.version, to: DelayedRewardAggregator.VERSION },
        '[DelayedRewardAggregator] 版本迁移',
      );
    }
