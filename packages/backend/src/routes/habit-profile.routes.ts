 ...
 * 提供用户学习习惯画像的API
 *
 * 注意：此路由已更新为使用统一的 userProfileService
 * 保持 API 接口不变以兼容前端
 *
...
router.post('/end-session', authMiddleware, async (req: AuthRequest, res, next) => {
  try {
    ...
    // 记录会话结束事件
    userProfileService.recordSessionEnd(userId, durationMinutes, session._count.answerRecords);
    ...
    // 持久化习惯画像
    const saved = await userProfileService.updateHabitProfile(userId);

    // 生成保存状态消息，区分"样本不足"和"保存失败"
    let habitProfileMessage: string | undefined;
    if (!habitProfile) {
      habitProfileMessage = '习惯画像保存失败，请稍后重试';
    } else if (habitProfile.samples.timeEvents < 10) {
      habitProfileMessage = `样本不足（当前${habitProfile.samples.timeEvents}/10），继续学习后将自动保存`;
    }
    ...
    res.json({
      ...
      data: {
        ...
        habitProfileSaved: !!habitProfile && habitProfile.samples.timeEvents >= 10,
        habitProfileMessage,
        preferredTimeSlots: habitProfile?.preferredTimeSlots || [],
      },
    });
  } catch (error) {
    next(error);
...
router.get('/', authMiddleware, async (req: AuthRequest, res, next) => {
  try {
    ...
    const realtime = profile.habitProfile;
...
router.post('/initialize', authMiddleware, async (req: AuthRequest, res, next) => {
  try {
    ...
    // 先重置，确保从干净状态开始
    userProfileService.resetUserHabit(userId);

    // 从历史记录初始化
    await userProfileService.initializeHabitFromHistory(userId);
    ...
    res.json({
      ...
      data: {
        ...
        profile: habitProfile
          ? {
              preferredTimeSlots: habitProfile.preferredTimeSlots,
              rhythmPref: habitProfile.rhythmPref,
              samples: habitProfile.samples,
            }
          : null,
      },
    });
