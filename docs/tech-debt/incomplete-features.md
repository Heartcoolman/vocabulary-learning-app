...
以下TODO注释实际上对应的功能已经实现，应该更新或删除：

### 1. word-mastery 路由
**位置**: `backend/src/routes/word-mastery.routes.ts:13`
```typescript
...
### 2. habit-profile 路由
**位置**: `backend/src/routes/habit-profile.routes.ts:12`
```typescript
...
### 3. AMAS batch-process 端点
**位置**: `backend/src/routes/amas.routes.ts:197`
```typescript
// ⚠️ 状态: 后端已实现，前端未调用 (TODO: 前端ApiClient缺少此方法)
```

**实际状态**:
- ✅ 前端方法: `ApiClient.batchProcessEvents()` 已实现（第1920行）
...
**建议**: 删除这些TODO注释，或者澄清"管理员UI"具体指什么高级功能
