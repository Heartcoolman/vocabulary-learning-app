...
│   │   └── vite.config.ts
│   │
│   ├── backend-rust/       # Rust 后端服务 (@danci/backend-rust)
│   │   ├── src/
│   │   │   ├── amas/       # AMAS 智能学习引擎
│   │   │   ├── routes/     # API 路由（35+ 模块）
│   │   │   ├── services/   # 业务服务
│   │   │   ├── db/         # 数据库层（PostgreSQL + SQLite 热备）
│   │   │   ├── auth/       # JWT 认证
│   │   │   ├── middleware/ # 中间件
│   │   │   └── workers/    # 后台任务
│   │   └── tests/          # 集成测试
...
> 完整 API 文档请参考 [packages/backend-rust/README.md](./packages/backend-rust/README.md)
...
```bash
cd packages/backend-rust && cargo run       # 开发服务器
cd packages/backend-rust && cargo build --release  # 生产构建
cd packages/backend-rust && cargo test      # 运行测试
```
...
| 文档                                                                                 | 说明             |
| ------------------------------------------------------------------------------------ | ---------------- |
| [packages/backend-rust/README.md](./packages/backend-rust/README.md)                 | 后端说明         |
| [packages/backend-rust/DEPLOYMENT.md](./packages/backend-rust/DEPLOYMENT.md)         | 部署指南         |
